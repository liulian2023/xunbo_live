 ...
 *
 * - 场景化配置参考：{@link } 与 {@link #setPushScene(int, boolean)} 您可以根据您的 App 使用设定不同的推流场景，SDK 内部会自动选择相关配置，让您可以快速搭建
 ...
 *
 * - 混响、变声、码率自适应、硬件加速等使用参考： {@link } 与 {@link }
 *
...
public class CameraPushImpl implements CameraPush, ITXLivePushListener {
    ...
    public void startPush() {
       ...
       /* if (TextUtils.isEmpty(tRTMPURL) || (!tRTMPURL.trim().toLowerCase().startsWith("rtmp://"))) {
            ...
            // 添加后台垫片推流参数
//            Bitmap bitmap = decodeResource(mContext.getResources(), R.drawable);
