...
//                spannableStringBuilder.setSpan(new ForegroundColorSpan(Color.YELLOW), 6, userName.length()+6, Spannable.SPAN_INCLUSIVE_INCLUSIVE);
    ...
    private String swichLevel(LiveMessageModel liveMessageModel) {
        ...
        String level1 = liveMessageModel.getLevel();
        if(Utils.isInt(level1)){
            int level = Integer.parseInt(StringMyUtil.isEmptyString(level1) ? "0": level1);
            if(level<=3&&level>1){
                fileName="join_lv1_5.svga";
            }else if(level>=4&&level<=6){
                fileName="join_lv6_10.svga";
            }else if(level>=7&&level<=9){
                fileName="join_lv11_15.svga";
            }else if(level>=10&&level<=12){
                fileName="join_lv16_20.svga";
            }else if(level>=13&&level<=15){
                fileName="join_lv21_25.svga";
            }else if(level>=16&&level<=18){
                fileName="join_lv26_30.svga";
            }else if(level>=19&&level<=21){
                fileName="join_lv31_35.svga";
            }else if(level>=22&&level<=24){
                fileName="join_lv36_40.svga";
            }else if(level>=25&&level<=27){
                fileName="join_lv41_45.svga";
            }else if(level>=28&&level<=30){
                fileName="join_lv46_50.svga";
            }
            return fileName;
        }else {
            return "join_lv1_5.svga";
        }
