...
public class FURenderer implements IFURenderer, IModuleManager {
    ...
    private void prepareDrawFrame() {
        ...
        LogUtils.error(TAG, "prepareDrawFrame.trackFace %d, ", trackFace);
        // 获取人体是否识别
    ...
    public int getFlag(boolean isFront){
        int flags = 0;
        if(isFront) {
            flags |= faceunity.FU_ADM_FLAG_FLIP_X;
        } else {
    ...
    public void setRotationMode( int cameraOri){
        switch (cameraOri) {
            case 90:
    ...
    public int setTextureID(int flags,int width, int height,int textureID){

        textureID= faceunity.fuRenderToTexture(textureID, width, height, mFrameId++, mItemsArray,flags);
